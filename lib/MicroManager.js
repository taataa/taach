...
var toRawPivot = function (mode, transformer, view) {
  ...
  // to MicroRecognizer, it looks like the parameter was not given.
}
...
var toSpaceEvent = function (ev, view) {
  ...
  return {
    ...
    totalTrans: new taaspace.InvariantTransform(ev.totaltrans, view),
    microTrans: new taaspace.InvariantTransform(ev.microtrans, view),
...
var MicroManager = function (element, transformer, view, emitter, mode) {
  ...
  this.recognizer.on('transformstart', function (event) {
    ...
    //   // This keeps transformer location the same in relation to the view.
    //   // As a goal, transforming the view with other fingers does not
    //   // move the transformer away from the gripping finger.
...
MicroManager.prototype.update = function (mode) {
  ...
  this.recognizer.setPivot(rawPivot)
}
