...
var Recognizer = function (element, handlers) {
  // Parameters:
  //   element
  //     HTMLElement to listen to
  //   handlers
  //     start
  //       function (firstPointers)
  //     move
  //       function (prevPointers, nextPointers)
  //     end
  //       function (lastPointers)
  //
  ...
  var onTouchStart = function (ev) {
    ...
    for (i = 0; i < cts.length; i += 1) {
      currPointers[cts[i].identifier] = [cts[i].pageX, cts[i].pageY]
    }

    if (!started) {
      started = true
      handlers.start(currPointers)
    }
  }
  ...
  var onMouseDown = function (ev) {
    ...
    if (!mouseDown) {
      ...
      currPointers['mouse'] = [ev.pageX, ev.pageY]
